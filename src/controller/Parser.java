...
public class Parser {
  ...
  private int createParseTree(int index,
    ...
    for (int i = 0; i < numInputs; i++) {
      ...
      if (index >= commandList.size()) {
        allData.setErrorMessage(head.getCommand().getClass().getSimpleName() + ": " +
                                errorResources.getString("noArgument"));
        error = true;
        return index;
      }
  ...
  private Double getNumInputs(ParseTreeNode<CommandInterface> node, ForObserverInterface allData) {
    Double numInputs =
        (double) Integer
            .parseInt(resources.getString(node.getCommand().getClass().getSimpleName()));
    if (node.getCommand() != null) {
      if (checkMatch("UserCommand", node)) {
        if (!commandTimesMap.containsKey(node.getCommand().getName()) &&
          ...
          try {
            ...
            allData.setErrorMessage(node.getCommandName() + " " +
                                    errorResources.getString("notFound"));
          }
        } else {
  ...
  public void printTreeInOrder(ParseTreeNode<CommandInterface> head) {
    ...
    System.out
        .println(head.getCommand().getClass().getSimpleName() + " " + head.getCommand().getValue() +
