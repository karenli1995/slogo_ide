...
public class CurrentTurtleState extends VBox implements Observer {
  ...
  public CurrentTurtleState(GUIManager guiManager,
                            Scene scene,
  ...
  public void update(Observable o, Object arg) {
    ...
    double currPosX =
        otherSlogoObj.getTurtleData(tabId)
            .getTurtle(otherSlogoObj.getTurtleData(tabId).getActiveTurtleID() - 1).getTrail()
            .getX();
    double currPosY =
        otherSlogoObj.getTurtleData(tabId)
            .getTurtle(otherSlogoObj.getTurtleData(tabId).getActiveTurtleID() - 1).getTrail()
    ...
    double currRotAngle =
        otherSlogoObj.getTurtleData(tabId)
            .getTurtle(otherSlogoObj.getTurtleData(tabId).getActiveTurtleID() - 1)
    ...
    double currPenPos =
        otherSlogoObj.getTurtleData(tabId)
            .getTurtle(otherSlogoObj.getTurtleData(tabId).getActiveTurtleID() - 1).getPen()
            .isDown();
    if (currPenPos == 1.0) {
      penPos = "Down";
    }
    ...
    if (otherSlogoObj.getTurtleData(tabId)
        .getTurtle(otherSlogoObj.getTurtleData(tabId).getActiveTurtleID() - 1)
        .getIsShowing() == true) {
      turtVis = "Visible";
    }
