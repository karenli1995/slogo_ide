...
public class TurtleScene extends TabPane implements Observer {
  ...
  public void setActiveTurtleID(int ID) {
    ...
    for (int i = 0; i < getCurrTab().getAllSlogoImages().size(); i++) {
      if ((!myController.getMyScene().getAllData().get(getIdOfTab()).getActiveTurtles()
          .contains(i)) && activeVis) {
  ...
  public void update(Observable o, Object arg) {
    ...
    for (int i = 0; i < turts.size(); i++) {
      ...
      try {
        ...
        addChildren(currSlogoImage.getMyImage());
      }
    ...
    if (otherSlogoObj.getTurtleData(tabId).getTurtle(0).getClearTrail() == true) {
      ...
      for (Object line : currLines) {
        removeChildren((Node) line);
      }
    }
