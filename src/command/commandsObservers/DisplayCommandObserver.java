...
public class DisplayCommandObserver implements Observer, Serializable {
  ...
  public void update(Observable o, Object arg) {
    ...
    Map<Display, List<Double>> map = DisplayCommands.getMap();
    ...
    if (map.get(Display.SETPC) != null) {
      for (int i = 0; i < data.activeTurtleListSize(); i++) {
        ...
        data.getAllTurtles().get(turt).getPen()
            .setColor(Color.web(myColors.get(map.get(Display.SETPC).get(0).intValue())));
      }
    }
    if (map.get(Display.SETPS) != null) {
      for (int i = 0; i < data.activeTurtleListSize(); i++) {
        int turt = data.activeTurtleListValue(i);
        data.getAllTurtles().get(turt).getPen()
            .setThickness(map.get(Display.SETPS).get(0).intValue());
      }
    }
    ...
    if (map.get(Display.SETPALETTE) != null) {
      String added =
          Color.rgb(map.get(Display.SETPALETTE).get(1).intValue(),
                    map.get(Display.SETPALETTE).get(2).intValue(),
                    map.get(Display.SETPALETTE).get(3).intValue())
              .toString();
