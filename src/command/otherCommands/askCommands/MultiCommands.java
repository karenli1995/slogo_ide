...
public abstract class MultiCommands extends Command {
  ...
  protected void createTempActivleList(List<ParseTreeNode<CommandInterface>> turtles,
                                       List<Integer> newActiveList) {
    try {
      for (int i = 1; i < turtles.size() - 1; i++) {
        newActiveList.add((int) turtles.get(i).getCommandValue());
      }
    } catch (Exception e) {
      turtleData.setErrorMessage(this.getClass().getSimpleName() + ": " +
