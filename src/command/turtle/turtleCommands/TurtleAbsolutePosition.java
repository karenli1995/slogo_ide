...
public abstract class TurtleAbsolutePosition extends Command {
	...
	protected double calculateDistanceBetweenTwoPoints(List<Double> newLocation) {
		...
		turtleData.getTurtle(0).getTrail().addCoord(new Point2D(x2, y2), turtleData.getTurtle(0).getPen().isDown());
	...
	protected void addToTrail(Point2D point) {
		...
		turtleData.getTurtle(0).getTrail().addCoord(point,turtleData.getTurtle(0).getPen().isDown());
